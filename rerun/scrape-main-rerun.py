...
# Get Mariadb cursor
conn = pymysql.connect(host='localhost', user='root', database='citation2', autocommit=True)
cursor = conn.cursor()
...
getnewProxy()
with open('rerun.csv', newline='') as csvfile:
    ...
    for row in csv_lines:
        ...
        while not success:
            ...
            print("Processing id: [" + id + "], title: [" + title + "], author: [" + author + "]" )

            # parsedKey = urllib.parse.quote(title)
            titleAuthor = title + " " + author
            parsedKey = urllib.parse.quote(titleAuthor)
